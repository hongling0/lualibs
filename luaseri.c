...
wb_string(struct write_block *wb, const char *str, int len) {
	if (len < MAX_COOKIE) {
		...
		if (len < 0x10000) {
			int n = COMBINE_TYPE(TYPE_LONG_STRING, 2);
			uint16_t x = (uint16_t) len;
			wb_push(wb, &n, 1);			
			wb_push(wb, &x, 2);
		} else {
			int n = COMBINE_TYPE(TYPE_LONG_STRING, 4);
...
_seri(lua_State *L, struct block *b) {
	...
	len -= 4;
	buffer = lua_newuserdata(L,len);// malloc(len);
	ptr = buffer;
	sz = len;
	if (len < BLOCK_SIZE - 4) {
...
static void
_seris(lua_State *L, struct block *b) {
	uint32_t len = 0;
	uint8_t * buffer,* ptr;
	size_t sz;
	memcpy(&len, b->buffer ,sizeof(len));
...
static int print_memery(lua_State* L)
{
...
_luaseri_unpack(lua_State *L) {
	...
	if (lua_isnoneornil(L,1)) {
		...
		buffer=(void*)luaL_checklstring(L,1,&len);
	}else{
...
int luaopen_luaseri(lua_State *L) {
	luaL_Reg l[] = {
		{ "pack", _luaseri_pack },
		{ "packs", _luaseri_packs },
		{ "unpack", _luaseri_unpack },
		{ "printmem", print_memery },
		{ NULL, NULL },
	};
	luaL_newlib(L,l);
	return 1;
